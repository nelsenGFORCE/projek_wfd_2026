    ...
    <div class="flex min-h-screen">
        ...
        <aside id="sidebar" class="fixed z-30 inset-y-0 left-0 w-64 bg-petra transform -translate-x-full lg:translate-x-0 lg:static lg:inset-auto transition-transform duration-200 ease-in-out">
            ...
            <nav class="mt-6 px-4 space-y-2">
                ...
                <a href="{{ route('admin.transactions.index') }}" class="relative flex items-center px-4 py-3 rounded-lg text-white hover:bg-petra-dark transition-colors duration-150 {{ request()->routeIs('admin.transactions.*') ? 'bg-petra-dark' : '' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/>
                    </svg>
                    Transactions
                    @if($pendingTransactionCount > 0)
                        <span class="absolute -top-1 -right-1 inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-red-600 rounded-full">{{ $pendingTransactionCount > 9 ? '9+' : $pendingTransactionCount }}</span>
                    @endif
                </a>
            </nav>
        </aside>
