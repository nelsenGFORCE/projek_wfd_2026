...
<div class="max-w-2xl mx-auto">
    ...
    <form action="{{ route('admin.schedules.update', $schedule->id) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
        @csrf @method('PUT')
        ...
        <div>
            ...
            <select name="movie_id" id="movie_id" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-petra focus:border-petra transition-colors @error('movie_id') border-red-500 @enderror" required>
                ...
                @foreach($movies as $movie)
                    <option value="{{ $movie->id }}" {{ old('movie_id', $schedule->movie_id) == $movie->id ? 'selected' : '' }}>{{ $movie->title }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="studio_id" id="studio_id" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-petra focus:border-petra transition-colors @error('studio_id') border-red-500 @enderror" required>
                ...
                @foreach($studios as $studio)
                    <option value="{{ $studio->id }}" {{ old('studio_id', $schedule->studio_id) == $studio->id ? 'selected' : '' }}>{{ $studio->studio_name }} ({{ $studio->capacity }} seats)</option>
                @endforeach
            </select>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="date" name="show_date" id="show_date" value="{{ old('show_date', $schedule->show_date->format('Y-m-d')) }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-petra focus:border-petra transition-colors @error('show_date') border-red-500 @enderror" required>
            ...
            <div>
                ...
                <input type="time" name="start_time" id="start_time" value="{{ old('start_time', \Carbon\Carbon::parse($schedule->start_time)->format('H:i')) }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-petra focus:border-petra transition-colors @error('start_time') border-red-500 @enderror" required>
        ...
        <div>
            ...
            <input type="number" name="ticket_price" id="ticket_price" value="{{ old('ticket_price', $schedule->ticket_price) }}" min="0" step="1000" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-petra focus:border-petra transition-colors @error('ticket_price') border-red-500 @enderror" required>
        ...
        <div class="flex items-center justify-end space-x-4 pt-4 border-t border-gray-200">
            ...
            <button type="submit" class="px-6 py-2.5 bg-petra text-white rounded-lg hover:bg-petra-dark transition-colors font-medium shadow-sm">Update Schedule</button>
        </div>
    </form>
</div>
@endsection
